...
export default function FitCheckPage() {
  ...
  const handleGarmentSelect = useCallback(async (garmentFile, garmentInfo) => {
    ...
    try {
      ...
      if (newOutfit.top) {
        topBlob =
          garmentInfo.id === newOutfit.top.id && garmentFile
            ? garmentFile
            : newOutfit.top.file; // dùng file gốc đã lưu
      }

      if (newOutfit.bottom) {
        bottomBlob =
          garmentInfo.id === newOutfit.bottom.id && garmentFile
            ? garmentFile
            : newOutfit.bottom.file;
      }

      if (newOutfit.accessory) {
        accBlob =
          garmentInfo.id === newOutfit.accessory.id && garmentFile
            ? garmentFile
            : newOutfit.accessory.file;
      }
